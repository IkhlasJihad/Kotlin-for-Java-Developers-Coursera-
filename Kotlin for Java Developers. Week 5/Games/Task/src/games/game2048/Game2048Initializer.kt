...
object RandomGame2048Initializer: Game2048Initializer<Int> {
     ...
     */
    override fun nextValue(gameBoard: GameBoard<Int?>): Pair<Cell, Int>? {
        // which cells are empty
        val emptyCells = gameBoard.getAllCells().filter { gameBoard[it] == null }.toMutableList()
        if (emptyCells.isEmpty()) return null
        
        // random empty cell
        val cellToFill = Random.nextInt (0, emptyCells.size)
        
        // generate the value and assign to the random cell
        return emptyCells[cellToFill] to generateRandomStartValue()
    }
}